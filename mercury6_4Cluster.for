...
c Reindex the surviving objects
	write(*,*) 'col!'
        call bcoord (time,jcen,nbound,nbig,nextra,h0,m,x,v,xh,vh,ngf,
     %        ngflag,opt)
        call mxx_elim (nbod,nbig,nbound,nextra,m,xh0,vh0,xh,vh,s,rho,
     %    rceh,rcrit,ngf,stat,id,mem,lmem,outfile(3),itmp)
c
...
c If something hit the central body, restore the coords prior to this step
      if (nhit.gt.0) then
	!flush close encounter arrays, since ID list is changing
        call mio_ce (time,tstart,rcen,rmax,nbod,nbig,m,stat,id,
     %    0,iclo,jclo,opt,stopflag,tclo,dclo,ixvclo,jxvclo,mem,lmem,
     %    outfile,nstored,0)
...
c If required, calculate the changeover distance used by hybrid algorithm
	...
	!stars
	do j = nboundbig + 1,nbig
	  rcrit(j) = (3.D0 * m(j) * 5.D2 * h * 5.D2 * h)**(1.D0 / 3.D0)
	enddo
      end if
c
...
c------------------------------------------------------------------------------
c
      subroutine mxx_elim (nbod,nbig,nbound,nextra,m,x0,v0,x,v,s,rho,
     %  rceh,rcrit,ngf,stat,id,mem,lmem,outfile,nelim)
c
      implicit none
      include 'mercury.inc'
c
...
c Find out how many objects are to be removed
	  ...
	  if ((j.le.nbig).and.(j.gt.nboundbig)) then
	    nextraelim = nextraelim + 1
        end if
      end do
      elim(nelim+1) = nbod + 1
c
...
c Update total number of bodies and number of Big bodies
      nbod = nbod - nelim
      nbig = nbig - nbigelim
      nbound = nbound - (nelim - nextraelim) 
      nextra = nextra - nextraelim
c
